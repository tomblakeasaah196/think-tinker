...
```php
// includes/config.php
define('SMTP_HOST', 'mail.think-tinker.com');  // or hosting SMTP
define('SMTP_PORT', 465);                       // SSL
define('SMTP_USER', 'no-reply@think-tinker.com');
define('SMTP_PASS', '***');
define('SMTP_FROM_NAME', 'Think & Tinker Kids Club');
define('SMTP_FROM_EMAIL', 'no-reply@think-tinker.com');
```
...
|---|---|---|
| id | INT AUTO_INCREMENT | PK |
| setting_key | VARCHAR(100) | UNIQUE. e.g. "business_name", "bank_account_name", "bank_account_number", "bank_name", "registration_fee", "assessment_fee", "session_rate_primary", "session_rate_secondary", "consultation_fee" |
| setting_value | TEXT | |
| updated_at | TIMESTAMP | |
...
| Key | Value |
|---|---|
| business_name | Think & Tinker Kids Club |
| bank_name | FCMB |
| bank_account_name | Think & Tinker Kids LTD |
| bank_account_number | (to be provided) |
| consultation_fee | 10000 |
| registration_fee | 8500 |
| assessment_fee | 15000 |
| session_rate_primary | 20000 |
| session_rate_secondary | 22000 |
| club_plan_trial | 8000 |
| club_plan_monthly | 30000 |
| club_plan_quarterly | 85000 |
| club_plan_biannual | 165000 |
...
- On submission:
  ...
  2. Page shows bank account details: Bank Name, Account Name, Account Number (from settings table).
  3. Text: "Please transfer ₦XX,XXX to the account above. Include your order number [TTK-ORD-XXXX] as payment reference. Once we confirm your payment, your order will be processed."
  4. Confirmation email sent with order summary and bank details.
...
- Pricing: consultation fee, registration fee, assessment fee, session rates, club plan prices.
- SMTP settings (for advanced users — pre-configured by developer).
- Logo upload.
...
**Location Management (future-ready):**
...
9. Configure `includes/config.php` with production database credentials and SMTP settings.
...
| Assessment Fee (child assessment, learning plan creation) for Nursery | ₦15,000 | Primary Level | ₦20,000 | Secondary Level | ₦22,000 |
...
## APPENDIX B: CURRENT STUDENT ROSTER AMONGST MANY OTHERS FOR CONTEXT
