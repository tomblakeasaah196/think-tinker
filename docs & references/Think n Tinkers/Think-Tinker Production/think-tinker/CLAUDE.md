...
All config lives in `.env` at project root. Loaded by `includes/config.php` using a simple parser. Never hardcode credentials.

```bash
cp .env.example .env
# Fill in database credentials, SMTP, and bank details
```
