        ...
        if (!r.data.conversations.length) {
            ...
            r.data.conversations.forEach(c => {
                html += `<div class="msg-convo-item ${c.conversation_id===Messenger.convoId?'active':''}" data-id="${c.conversation_id}">
                    ...
                    <div style="flex:1;min-width:0;"><div style="font-weight:700;font-size:0.8125rem;">Think & Tinker</div><div style="font-size:0.75rem;color:#999;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${TT.escHtml(c.last_message||'')}</div></div>
